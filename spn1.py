...
#y=0010    0100    1011   1001       
    ...
    def encrypt_data(self, data, key, rounds):
        #bkey=format(bin(key)[2:].zfill(16))
    ...
    """def round_keys_to_decrypt(self, key):
        ...
        L = []
        # код
       # L.append(int(str(K[4]),2))
       # L.append(int(str(self.pbox(K[3])),2))
       # L.append(int(str(self.pbox(K[2])),2))
       # L.append(int(str(self.pbox(K[1])),2))
        #L.append(int(str(K[0]),2))

        L.append((bin(int(K[4]))[2:].zfill(16)))
        L.append((bin(int(self.pbox(K[3])))[2:].zfill(16)))
        L.append((bin(int(self.pbox(K[2])))[2:].zfill(16)))
        L.append((bin(int(self.pbox(K[1])))[2:].zfill(16)))
        L.append((bin(int(K[0]))[2:].zfill(16)))
        return L"""
    ...
    def decrypt_ctr(self, data_c, key, rounds, initV):
        ...
        data = []
        for m in data_c:
            x=self.encrypt(initV,rk,rounds)       #self, p, rk, rounds
            x=x^m
            initV+=1
            data.append(x)
